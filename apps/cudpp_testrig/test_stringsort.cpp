...
int verifyPackedStringSort(unsigned int *valuesSorted,
					       unsigned int* packedStringVals, size_t numElements, unsigned int stringSize, unsigned char termC)
{
	int retval = 0;
	...
	for(unsigned int i = 0; i < numElements-1; ++i)
	{
		...
		while(c1 == c2 && (c1&255) != termC && (c2&255) != termC && add1 < stringSize && add2 < stringSize);
                  ...
                  testrigOptions testOptions, bool quiet)
{
    ...
    CUDA_SAFE_CALL(cudaMalloc((void **) &d_alignedKeys, numElements*sizeof(unsigned int)));	
	...
	CUDA_SAFE_CALL(cudaMalloc((void**)&d_packedStringVals, packedStringSize*sizeof(unsigned int)));
    CUDA_SAFE_CALL(cudaMemcpy(d_stringVals, stringVals, stringSize*sizeof(unsigned char), cudaMemcpyHostToDevice));
	CUDA_SAFE_CALL(cudaMemcpy(d_packedStringVals, packedStringVals, packedStringSize*sizeof(unsigned int), cudaMemcpyHostToDevice));
	...
	for (unsigned int k = 0; k < numTests; ++k)
    {
        ...
        {		
            ...
            CUDA_SAFE_CALL( cudaMemcpy(d_address, h_valAligned, tests[k] * sizeof(unsigned int), cudaMemcpyHostToDevice) );            
			CUDA_SAFE_CALL( cudaMemcpy(d_alignedKeys, h_alignedKeys, tests[k] * sizeof(unsigned int), cudaMemcpyHostToDevice) );            
            CUDA_SAFE_CALL( cudaEventRecord(start_event, 0) );

			
            cudppStringSortAligned(plan, d_alignedKeys, d_address, d_packedStringVals, tests[k], packedStringSize);
		...
		retval += verifyPackedStringSort(h_valuesSorted,
					  packedStringVals, tests[k], packedStringSize, 0);
