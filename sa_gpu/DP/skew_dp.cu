...
__device__ bool UniqueRank(unsigned int*key, int size)
{
...
__global__ void ComputeSA(unsigned int* str, unsigned int* keys_sa, int str_length, CudaContext& context)
{
    ...
    for(int i=0;i<mod_1+mod_2;i++)
       {
            ...
            if (i==0) {j=1; rank[i]=j;}
            else if( (str[keys_srt_12[i]-1]==str[keys_srt_12[i-1]-1]) && (str[keys_srt_12[i]]==str[keys_srt_12[i-1]]) && (str[keys_srt_12[i]+1]==str[keys_srt_12[i-1]+1]) ) { rank[i]=j;    }
            else {j++;  rank[i]=j;  }
    ...
    for(int i=0;i<mod_1+mod_2;i++)
       {
            ...
            if (i==0) {j=1; keys_sa_12[keys_srt_12[i]-1]=j; }
            else if(  (str[keys_srt_12[i]-1]==str[keys_srt_12[i-1]-1]) && (str[keys_srt_12[i]]==str[keys_srt_12[i-1]]) && (str[keys_srt_12[i]+1]==str[keys_srt_12[i-1]+1]) ) {keys_sa_12[keys_srt_12[i]-1]=j;    }
            else {j++; keys_sa_12[keys_srt_12[i]-1]=j;   }
   //keys_srt_12[i]: sorted index       
       }
...
/////recursive part///////
    ...
    ComputeSA<<< 1, 1, 0, s >>>(keys_rank_12,keys_srt_12,mod_1+mod_2-1,context);
    cudaStreamDestroy( s );
...
//////////////////////////// merge sort//////////////////////////////////
    ...
    // aVals=keys_srt_12; bVals=keys_srt_3;
     ...
     for(int j=0;j<mod_3;j++) bValsHost[j]=keys_srt_3[j];
...
void runComputeSA(unsigned int* str, unsigned int* keys_sa, int str_length, CudaContext& context)
{
  MGPU_MEM(unsigned int) d_str=context.Malloc((const unsigned int*) str, (size_t) (str_length+3));
  MGPU_MEM(unsigned int) d_keys_sa=context.Malloc((const unsigned int*) keys_sa, (size_t) (str_length+1));
  ComputeSA<<< 1, 1 >>>(d_str,d_keys_sa,str_length,context);
