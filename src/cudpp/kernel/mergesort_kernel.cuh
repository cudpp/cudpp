...
void blockWiseSort(T *A_keys, unsigned int* A_values, int blockSize, size_t totalSize)
{
	...
	//Bitonic Sort - Begin (modified to be stable)
	//Sort first 8 values
	compareSwapVal<T>(myKey[0], myKey[1], myValue[0], myValue[1]);	
	compareSwapVal<T>(myKey[2], myKey[3], myValue[2], myValue[3]);
	compareSwapVal<T>(myKey[0], myKey[2], myValue[0], myValue[2]);
	compareSwapVal<T>(myKey[1], myKey[3], myValue[1], myValue[3]);
	...
	compareSwapVal<T>(myKey[4], myKey[6], myValue[4], myValue[6]);
	compareSwapVal<T>(myKey[5], myKey[7], myValue[5], myValue[7]);
	compareSwapVal<T>(myKey[5], myKey[6], myValue[5], myValue[6]);
	compareSwapVal<T>(myKey[0], myKey[4], myValue[0], myValue[4]);
	compareSwapVal<T>(myKey[1], myKey[5], myValue[1], myValue[5]);
	compareSwapVal<T>(myKey[2], myKey[6], myValue[2], myValue[6]);
	compareSwapVal<T>(myKey[3], myKey[7], myValue[3], myValue[7]);
	compareSwapVal<T>(myKey[2], myKey[4], myValue[2], myValue[4]);
	compareSwapVal<T>(myKey[3], myKey[5], myValue[3], myValue[5]);
	compareSwapVal<T>(myKey[1], myKey[2], myValue[1], myValue[2]);
...
//#define DVAL1 2110853304
//#define DVAL2 2110853304
...
void simpleMerge_lower(T *A_keys, unsigned int* A_values, T *A_keys_out, unsigned int *A_values_out, int sizePerPartition, int size)
{
	...
	{		
		...
		//if(myKey[1] >= DVAL1 && myKey[1] <= DVAL2 && sizePerPartition == 2048)
		//	printf("lower myKey1 %u %d %d\n", myKey[1], globalCAddress + 1 + index, aIndex+tid*depth+1);
		...
		//	printf("lower myKey0 %u %u %u %d %d\n", myKey[0], localMinB, localMaxB, globalCAddress + index, aIndex+tid*depth);
...
#pragma unroll
	...
	{	
		...
		{	
			...
			{
		//		if(myKey[1] >= DVAL1 && myKey[1] <= DVAL2 && sizePerPartition == 2048)
		//	printf("higher myKey1 %u %u %u %u %d %d %d\n", myKey[1], localMinB, nextMaxB, BKeys[index-1], globalCAddress + 1 + index, aIndex+tid*depth+1, bIndex+index);
				A_keys_out[globalCAddress + index + 1] =  myKey[1]; A_values_out[globalCAddress + index + 1] =  myValue[1];	}			
...
void findMultiPartitions(T *A, int splitsPP, int numPartitions, int partitionSize,  int* partitionBeginA, int* partitionSizesA, int sizeA)
{
	...
	//printf("myStartA %d myEndA %d myStartRange %d myEndRange %d %u %u\n", myStartA, myEndA, myStartRange, myEndRange, myStartSample, myEndSample);
...
#define DVAL1  -339783865857699910000000000000000000000.000000
#define DVAL2  -339783865857699910000000000000000000000.000000
...
#pragma unroll
	...
	do{				
		...
		{		
			...
			//if(myKey[0] >= DVAL1 && myKey[0] <= DVAL2)
			//	printf("lower mykey0 %f cmpV %f minB %f maxB %f nextB %f index %d \n start %d bIndex %d aIndex %d index %d d*tid%d entPartSize %d localPartSize %d \n", myKey[0], cmpValue, localMinB, localMaxB, nextMaxB, globalCAddress+index, 
			//	myStartIdxC, bIndex, aIndex, index, depth*tid, entirePartitionSize, localBPartSize);
			...
			//if(myKey[1] >= DVAL1 && myKey[1] <= DVAL2)
			//	printf("lower myKey1 %f %f %f %f %f %d %d %d %d\n", myKey[1], cmpValue, localMinB, localMaxB, nextMaxB, globalCAddress+index+1, index,localBPartSize, bIndex);
			...
			//if(myKey[2] >= DVAL1 && myKey[2] <= DVAL2)
			//	printf("lower myKey2 %4.2f %4.2f %4.2f %4.2f %4.2f %4.2d %4.2d %4.2d %4.2d\n", myKey[2], cmpValue, localMinB, localMaxB, nextMaxB, globalCAddress+2+index, index,localBPartSize, bIndex);

			//if(myKey[3] >= DVAL1 && myKey[3] <= DVAL2)
			//	printf("lower myKey3 %4.2f %4.2f %4.2f %4.2f %4.2f %d %d %d d\n", myKey[3], cmpValue, localMinB, localMaxB, nextMaxB, globalCAddress+3+index, index,localBPartSize, bIndex);
...
#pragma unroll
	...
	//if(tid == 0)
	//	printf("nextM %u localM %u localMi %u pSize %d tid %d\n", nextMaxB, localMaxB, localMinB, localBPartSize, myId);
	do
	{
		...
		/*if(myKey[0] >= DVAL1 && myKey[0] <= DVAL2)
			printf("higher myKey0 %u %u %u %d %d\n", myKey[0], localMaxB, nextMaxB, myStartIdxC + index + bIndex + aIndex + tid*depth, index);
		if(myKey[1] >= DVAL1 && myKey[1] <= DVAL2)
			printf("higher myKey1 %u %u %u %d %d\n", myKey[1], localMaxB, nextMaxB, myStartIdxC + index + bIndex + aIndex + tid*depth+1, index);
		if(myKey[2] >= DVAL1 && myKey[2] <= DVAL2)			
			printf("higher myKey2 %u %u %u %u %u %d %d %u %u %d %d\n", myKey[2], cmpValue, localMinB, localMaxB, nextMaxB, myStartIdxC + index + bIndex + aIndex + tid*depth+2, index, myKey[0], myKey[3], localAIndex+2, localAPartSize);
		if(myKey[3] >= DVAL1 && myKey[3] <= DVAL2)
				printf("higher myKey3 %u %u %u %u %u %d %d\n", myKey[3], cmpValue, localMinB, localMaxB, nextMaxB, myStartIdxC + index + bIndex + aIndex + tid*depth+3, index);
				*/

		//if(myKey[2] >= DVAL1 && myKey[2] <= DVAL2)
		//	printf("%u %u %u\n", myKey[0], myKey[1], myKey[2]);
		//if(myKey[2] >= DVAL1 && myKey[2] <= DVAL2)
		//	printf("%u <= %u %u >= %u %d < %d %d %d %d %u %d\n", myKey[0], nextMaxB, myKey[3], localMinB, localAIndex, localAPartSize, bIndex+INTERSECT_B_BLOCK_SIZE_multi, localBPartSize, partitionEndB,  A_keys[myStartIdxB + bIndex + localBPartSize], myStartIdxB);
		if((myKey[0] <= nextMaxB) && myKey[depth-1] >= localMinB && localAIndex < localAPartSize)		
		{	
			...
			//if(myKey[0] >= DVAL1 && myKey[0] <= DVAL2)
			//	printf("higher myKey0 %u max %u next %u min %u address %d \n start %d aInd %d bInd %d ind %d localind %d partSize %d\n", myKey[0], localMaxB, nextMaxB, localMinB, myStartIdxC + index + bIndex + aIndex + tid*depth, myStartIdxC, aIndex, bIndex, index, tid*depth, entirePartitionSize);
			...
			//if(myKey[1] >= DVAL1 && myKey[1] <= DVAL2)
			//	printf("higher myKey1 %f %f cmp %f max %f next %f min %f address %d \n start %d aInd %d bInd %d ind %d localind %d partSize %d %d %d %d\n", myKey[1], myKey[2], cmpValue, localMaxB, nextMaxB, localMinB, myStartIdxC + index + bIndex + aIndex + tid*depth+1, myStartIdxC, aIndex, bIndex, index, tid*depth+1, myStartIdxB, localBPartSize, myStartIdxA, localAPartSize);
			if(localAIndex+2 < localAPartSize)
			...
			//if(myKey[2] >= DVAL1 && myKey[2] <= DVAL2 && entirePartitionSize == 8192 && localAIndex+2<localAPartSize)
			//	printf("higher myKey2 %f max %f next %f min %f address %d \n start %d aInd %d bInd %d ind %d localind %d partSize %d\n", myKey[2], localMaxB, nextMaxB, localMinB, myStartIdxC + index + bIndex + aIndex + tid*depth+2, myStartIdxC, aIndex, bIndex, index, tid*depth+2, entirePartitionSize);
			...
			//if(myKey[3] >= DVAL1 && myKey[3] <= DVAL2)
			//	printf("higher myKey3 %u max %u next %u min %u address %d \n start %d aInd %d bInd %d ind %d localind %d partSize %d\n", myKey[3], localMaxB, nextMaxB, localMinB, myStartIdxC + index + bIndex + aIndex + tid*depth+3, myStartIdxC, aIndex, bIndex, index, tid*depth+3, entirePartitionSize);
