...
#pragma unroll
	...
	while (mult < steps)
	{
	   ...
	   __threadfence();
		...
		//if(mult == 64 && tmpV >= INB1 && tmpV <= INB2  && blockIdx.x == 0)
		//	printf("tmpV %d tid*depth+2 %d %d\n", tmpV, tid*depth+2, saveValue[2]);
		__threadfence();
        __syncthreads();
		...
		//if(mult == 64 && tmpV >= INB1 && tmpV <= INB2  && blockIdx.x == 0)
		//	printf("tmpV %d tid*depth+2 %d %d %d %d\n", tmpV, tid*depth+2, Aval[2], saveValue[2], threadIdx.x);
		//if(tid == 0 && Aval[0] == 943 && mult == 64)
		//	printf("values saved were %d\n", in[943]);
		__threadfence();
        __syncthreads();   
						 ...
						 unsigned int* partitionBeginB, unsigned int* partitionSizesB, size_t size, size_t stringSize)
{
	...
	first = mid; //The start from last time
	last = myEndRange;
	mid = (first + last)/2;	
